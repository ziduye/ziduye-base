...
if (SpeechRecognition = root.SpeechRecognition || root.webkitSpeechRecognition || root.mozSpeechRecognition || root.msSpeechRecognition || root.oSpeechRecognition, SpeechRecognition && voice_command) {
    $.root_.on("click", '[data-action="voiceCommand"]',
    function(a) {
        $.root_.hasClass("voice-command-active") ? $.speechApp.stop() : ($.speechApp.start(), $("#speech-btn .popover").fadeIn(350)),
        a.preventDefault()
    }),
    ...
    $.speechApp = function(a) {
        return a.start = function() {
            ...
            smartSpeechRecognition ? (smartSpeechRecognition.start(), $.root_.addClass("voice-command-active"), $.speechApp.playON(), voice_localStorage && localStorage.setItem("sm-setautovoice", "true")) : alert("speech plugin not loaded")
        },
        a.stop = function() {
...
} else $("#speech-btn").addClass("display-none"); (function(a) {
    "use strict";
    if (!SpeechRecognition) return root.smartSpeechRecognition = null,
    a;
    var b, c, d = [],
    e = {
        "start": [],
    ...
    k = /[\-{}\[\]+?.,\\\^$|#]/g,
    l = function(a) {
        return a = a.replace(k, "\\$&").replace(g, "(?:$1)?").replace(i,
        function(a, b) {
            return b ? a: "([^\\s]+)"
    ...
    n = function() {
        o() || root.smartSpeechRecognition.init({},
        !1)
    },
    o = function() {
        return b !== a
    ...
    root.smartSpeechRecognition = {
        "init": function(g, h) {
            ...
            b.onstart = function() {
                m(e.start),
                debugState && (root.console.log("%c \u2714 SUCCESS: User allowed access the microphone service to start ", debugStyle_success), root.console.log("Language setting is set to: " + b.lang, debugStyle)),
            ...
            b.onresult = function(a) {
                ...
                for (var b, c = a.results[a.resultIndex], f = 0; f < c.length; f++) {
                    ...
                    h = d.length; h > g; g++) {
                        ...
                        if (i) {
                            var j = i.slice(1);
                            debugState && (root.console.log("command matched: %c" + d[g].originalPhrase, debugStyle), j.length && root.console.log("with parameters", j)),
...
var autoStart = function() {
    smartSpeechRecognition.addCommands(commands),
    smartSpeechRecognition ? (smartSpeechRecognition.start(), $.root_.addClass("voice-command-active"), voice_localStorage && localStorage.setItem("sm-setautovoice", "true")) : alert("speech plugin not loaded")
};
